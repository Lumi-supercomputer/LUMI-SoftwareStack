if os.getenv( '_LUMI_LMOD_DEBUG' ) ~= nil then
  LmodMessage( 'DEBUG: In ' .. myModuleFullName () .. ' (linked to partition_stack/PrgEnv/PrgEnv-cray.lua), the LMOD mode is ' .. mode() )
end
...
if mode() == load then
  ...
  partition = os.getenv( '_LUMI_PrgEnv_cray_STATE_PARTITION' )
  if partition == nil then
    partition = os.getenv( 'LUMI_OVERWRITE_PARTITION' )
    if partition == nil then
...
setenv( '_LUMI_PrgEnv_cray_STATE_PARTITION', partition )
