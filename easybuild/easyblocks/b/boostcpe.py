...
class EB_BoostCPE(EasyBlock):
    """Support for building Boost."""

    def __init__(self, *args, **kwargs):
        """Initialize Boost-specific variables."""
        super(EB_BoostCPE, self).__init__(*args, **kwargs)
    ...
    def patch_step(self):
        ...
        super(EB_BoostCPE, self).patch_step()
    ...
    def prepare_step(self, *args, **kwargs):
        ...
        super(EB_BoostCPE, self).prepare_step(*args, **kwargs)
    ...
    def configure_step(self):
        ...
        if self.toolchain.options.get('usempi', True) and not self.cfg['boost_mpi']:
            self.cfg['boost_mpi'] =  True
            self.log.info('EB_BoostCPE: Turning boost_mpi on as usempi is True.')

        # Derive the bootstrap and build toolsets from the toolchain,
        # but this is only needed if toolset is not set.
        if self.cfg['toolset'] is None:
            if self.toolchain.comp_family() == toolchain.INTELCOMP:
                def_bootstrap_toolset = 'intel-linux'
                def_build_toolset = 'intel'
                self.log.info('EB_BoostCPE: Default bootstrap toolset intel-linux and build toolset intel')
            elif self.toolchain.comp_family() == toolchain.GCC:
                def_bootstrap_toolset = 'gcc'
                def_build_toolset = 'gcc'
                self.log.info('EB_BoostCPE: Default bootstrap and build toolset both gcc')
            elif self.toolchain.comp_family() == toolchain.CLANG:
                def_bootstrap_toolset = 'clang'
                def_build_toolset = 'clang'
                self.log.info('EB_BoostCPE: Default bootstrap and build toolset both clang')
            elif self.toolchain.comp_family() == toolchain.CCE:
                def_bootstrap_toolset = 'gcc'
                def_build_toolset = 'clang'
                self.log.info('EB_BoostCPE_CSCS: Setting (auto-detect) gcc bootstrap toolset for the CCE compiler as Cray clang generates errors. This will use the system GCC.')
                self.log.info('EB_BoostCPE: Default bootstrap toolset gcc (system GCC) and build toolset clang (CCE compiler)')
            elif self.toolchain.comp_family() == toolchain.AOCC:
                def_bootstrap_toolset = 'clang'
                def_build_toolset = 'clang'
                ...
                def_bootstrap_toolset = 'gcc'
                def_build_toolset = 'clang'
                self.log.info('EB_BoostCPE: Default bootstrap toolset (system GCC) and build toolset clang (AMD ROCm compiler)')
        ...
        if toolset is None:
            toolset = def_bootstrap_toolset
        # Derive the build toolset
        if self.cfg['toolset']:
            # Default value for build_toolset is derived from toolset
            self.toolset = self.cfg['build_toolset'] or re.sub('-linux$', '', self.cfg['toolset'])
        else:
            self.toolset = self.cfg['build_toolset'] or re.sub('-linux$', '', def_build_toolset)
        self.log.info('EB_BoostCPE: toolset for bootstrapping: %s, toolset for building: %s' % (toolset, self.toolset))
        ...
        if self.cfg['boost_mpi']:
            ...
            if self.toolchain.toolchain_family() == toolchain.CRAYPE:
                ...
                # These settings are based on the boost.sh script from https://github.com/Cray/pe-scripts
                user_config.append("using mpi : : <include>. : srun ;")
            else:
...
#                    custom_paths['files'].append(os.path.join('lib', 'libboost_mpi%s.%s' % (lib_mt_suffix, shlib_ext)))
    ...
    def make_module_extra(self):
        ...
        txt = super(EB_BoostCPE, self).make_module_extra()
