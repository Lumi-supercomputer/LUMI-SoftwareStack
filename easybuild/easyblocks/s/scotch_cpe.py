...
class EB_SCOTCH_CPE(EasyBlock):
    ...
    def extra_options(extra_vars=None):
        ...
        extra_vars = {
            'threadedmpi': [None, "Use threaded MPI calls (dangerous as the user application needs to use MPI_Init_thread).", CUSTOM],
            'rename_parser': [False, "Build with SCOTCH_RENAME_PARSER, default: False", CUSTOM],
            'metis_prefix': [False, "Prefix the name of the METIS compatibiity routines with SCOTCH_, default: False", CUSTOM]
        }
    ...
    def configure_step(self):
        ...
        if comp_fam == toolchain.INTELCOMP:  # @UndefinedVariable
            ...
            # Note that the following file has to be provided by the user
            # via the EasyConfig file and does not come with SCOTCH.
            makefilename = 'Makefile.inc.CPE'
        else:
        ...
        makefile_template = os.path.join(srcdir, 'Make.inc', makefilename)
        copy_file(makefile_template, makefile_inc)
        # TODO: Note: "successfully" below is an overstatement as it does not test for the result
        # of copy_file...
        self.log.info("Successfully copied %s to %s" % (makefile_template,makefile_inc))

        # the default behaviour of these makefiles is still wrong
        # e.g., compiler settings, and we need -lpthread
        # Note that not all of these regular expressions are needed all the time, but they
        # do no harm.
        # The first three edits are useless anyway as these three variables are defined
        # on the command line of make which should overwrite the values specified in the
        # Makefile.
        regex_subs = [
            (r"^CCS\s*=.*$", "CCS\t= $(CC)"),
            (r"^CCP\s*=.*$", "CCP\t= $(MPICC)"),
            ...
            (r"^LDFLAGS\s*=(?P<ldflags>.*$)", r"LDFLAGS\t=$(EXTRA_LD_FLAGS) \g<ldflags>"),
        ]
    ...
    def build_step(self):
        ...
        # Add support for GZ-compressed graphs if zlib is available.
        if get_software_root('zlib'):
            cflags += ' -DCOMMON_FILE_COMPRESS_GZ'
            extra_ld_flags += ' -L${EBROOTZLIB}/lib -lz'

        # Add support for bz2-compressed graphs if bzip2 is available
        if get_software_root('bzip2'):
            cflags += ' -DCOMMON_FILE_COMPRESS_BZ2'
            extra_ld_flags += ' -L${EBROOTBZIP2}/lib -lbz2'

        # Add support for LZMA-compressed files if the XZ module is available
        if get_software_root('XZ'):
            cflags += ' -DCOMMON_FILE_COMPRESS_LZMA'
            extra_ld_flags += ' -L${EBROOTXZ}/lib -llzma'

        # In older versions of SCOTCH COMMON_PTHREAD was used instead of
        # COMMON_PTHREAD_FILE to activate a separate thread for handling of
        # compressed files
        cflags += ' -DCOMMON_PTHREAD_FILE'

        # Use the same seed for random numbers at every execution.
        # This is the preferred setting according to the developers.
        cflags += ' -DCOMMON_RANDOM_FIXED_SEED'

        # Best to leave this defined to avoid name clashes with other
        # libraries.
        cflags += ' -DSCOTCH_RENAME'

        # Also rename the parser routines
        if self.cfg['rename_parser']:
            cflags += ' -DSCOTCH_RENAME_PARSER'

        # Prefix the names of the compatibility routines with SCOTCH_
        if self.cfg['metis_prefix']:
        ...
        elif self.toolchain.comp_family() == toolchain.CCE:  # @UndefinedVariable
            cflags += " -Drestrict=__restrict"
        elif self.toolchain.comp_family() == toolchain.AOCC:  # @UndefinedVariable
            cflags += " -Drestrict=__restrict"
    ...
    def sanity_check_step(self):
        ...
        super(EB_SCOTCH_CPE, self).sanity_check_step(custom_paths=custom_paths, custom_commands=custom_commands)
