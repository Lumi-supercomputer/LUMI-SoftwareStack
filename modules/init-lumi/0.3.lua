...
--
-- Add additional target modules.
--
prepend_path( 'MODULEPATH', pathJoin( LUMI_root, 'modules/craype-targets' ) )

--
-- Add additional directories to the MODULEPATH
...
if mode() == 'load' or mode() == 'show' then
    ...
    if os.getenv( '_LUMI_INIT_FIRST_LOAD' ) == nil and is_interactive() then
        ...
        -- Check quota and print the warnings.
        local quota_message = capture( pathJoin( LUMI_root, repo, 'scripts/lumi-check-quota.sh' ) )
        if quota_message ~=  nil and quota_message ~= '' then
            LmodMessage( '\nThere are storage quota and storage billing unit warnings:\n' .. quota_message )
        end
