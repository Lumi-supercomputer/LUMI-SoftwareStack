if os.getenv( '_LUMI_LMOD_DEBUG' ) ~= nil then
  LmodMessage( 'DEBUG: In ' .. myModuleFullName () .. ' (linked to partition_stack/PrgEnv/PrgEnv-gnu.lua), the LMOD mode is ' .. mode() )
end
...
if mode() == load then
  ...
  partition = os.getenv( '_LUMI_PrgEnv_gnu_STATE_PARTITION' )
  if partition == nil then
    partition = os.getenv( 'LUMI_OVERWRITE_PARTITION' )
    if partition == nil then
...
whatis( 'PrgEnv-gnu loads a collection of modules together defining the Cray Programming Environment with the GNU compilers.'  )

help( [==[
PrgEnv-gnu loads a collection of modules together defining the Cray Programming
Environment with the GNU compilers.
...
setenv( '_LUMI_PrgEnv_gnu_STATE_PARTITION', partition )
